...
public class Int32Compression
{
	...
	public static int[] read_Int32CDP( ByteReader reader ) throws IOException
	{
		...
		if ( DebugInfo.debugCodec ) {
			BundleAccessor.getLogger().info( "Codec Type: {}", codecType ); //$NON-NLS-1$
		}
		...
		if ( (codecType == HUFFMAN_CODEC) || (codecType == ARITHMETIC_CODEC) ) {
			...
			if ( outOfBandValueCount > 0 ) {
				...
				if ( DebugInfo.debugCodec ) {
					...
					BundleAccessor.getLogger().info( "** Out Of Band Values **" ); //$NON-NLS-1$
				}
				...
				if ( DebugInfo.debugCodec ) {
					BundleAccessor.getLogger().info( "Out Of Band Values ({})", outOfBand.length ); //$NON-NLS-1$
		...
		if ( codecType != NULL_CODEC ) {
			...
			if ( DebugInfo.debugCodec ) {
				BundleAccessor.getLogger().info( "" ); //$NON-NLS-1$
				BundleAccessor.getLogger().info( "Code Text Length: {}", codeTextLength ); //$NON-NLS-1$
				BundleAccessor.getLogger().info( "Value Element Count: {}", valueElementCount ); //$NON-NLS-1$
			}

			if ( probContext.getContextTableCount() == 1 || probContext.getContextTableCount() == 0 ) {
				...
				if ( DebugInfo.debugCodec ) {
					BundleAccessor.getLogger().info( "Symbol Count: {}", numSymbolsToRead ); //$NON-NLS-1$
				}
			}
			else {
				BundleAccessor.getLogger().info( "Problem with number of tables in Prob Ctxt -{} tables", probContext.getContextTableCount() ); //$NON-NLS-1$
			}
		}

		// residualValues is the vector of decoded symbols
		int[] residualValues = getResidualValues( reader, codecType, codeTextLength, probContext, numSymbolsToRead, valueElementCount );

		if ( DebugInfo.debugCodec ) {
			BundleAccessor.getLogger().info( "Residual Values I32 ({}):", residualValues.length ); //$NON-NLS-1$
	...
	public static int[] read_VecI32_Int32CDP( ByteReader reader, PredictorType predType ) throws IOException
	{
		// residualValues is the vector of decoded symbols
		int[] residualValues = read_Int32CDP( reader );
		Predictors.unpackResidualsOverwrite( residualValues, predType );

		return residualValues;
	}

	public static U32Vector read_VecU32_Int32CDP( ByteReader reader, PredictorType predType ) throws IOException
	{
...
//        for (int i = 0; i < primalValues.length; i++)
	...
	public static byte[] getCodeText( ByteReader reader, int codeTextLength ) throws IOException
	{
	...
	// algorithm
	public static int[] getResidualValues( ByteReader reader, int codecType, int codeTextLength, Int32ProbCtxts probContext, int numSymbolsToRead, int valueElementCount ) throws IOException
	{
		int[] residualValues;
		...
		if ( codecType == NULL_CODEC ) {
			...
			try {
				...
				if ( codecType == BITLENGTH_CODEC ) {
					...
					residualValues = decompressByHuffman( codeText, codeTextLength, probContext, numSymbolsToRead );
				}
				else if ( codecType == ARITHMETIC_CODEC ) {
					ArithmeticCodec codec = new ArithmeticCodec();
					residualValues = codec.decode( codeText, codeTextLength, probContext, numSymbolsToRead, valueElementCount );
				}
				else {
					residualValues = null;
				}
...
//            }
	 ...
	 */
	private static int[] decompressByBitLength( byte[] encodedBytes, int codeTextLength, int numSymbolsToRead )
	{
	...
	private static int[] decompressByHuffman( byte[] encodedBytes, int codeTextLength, Int32ProbCtxts probCtxt, int numSymbolsToRead )
	{
			...
			if ( node.isLeaf() ) {
				...
				if ( symbol == -2 ) {
					if ( cptOutOfBand < outofBandValues.length ) {
						...
						BundleAccessor.getLogger().error( "out of out of band values!" ); //$NON-NLS-1$
					}
				}
				else {
